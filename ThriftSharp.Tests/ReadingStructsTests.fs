...
let (-->) (fieldData, typeId) (expected: 'a) =
    ...
    let data = 
        [StructHeader "Struct"; FieldHeader (1s, "Field", tid (byte typeId))]
      @ fieldData 
...
let fails<'field> typeId fieldData =
    ...
    let data = [StructHeader "Struct"; FieldHeader (1s, "Field", tid (byte typeId))] @ fieldData @ [FieldEnd; FieldStop; StructEnd] 
    let m = MemoryProtocol(data)
    let thriftStruct = ThriftAttributesParser.ParseStruct(typ.GetTypeInfo())
...
type __() =
    ...
    // Collection fields
    [<Test>] member __.``List``() =         [ListHeader (1, tid 8uy); Int32 1; ListEnd]                     -- 15 --> List([1])
    [<Test>] member __.``List, empty``() =  [ListHeader (0, tid 8uy); ListEnd]                              -- 15 --> List<int>()
    [<Test>] member __.``Set``() =          [SetHeader (1, tid 8uy); Int32 1; SetEnd]                       -- 14 --> HashSet([1])
    [<Test>] member __.``Set, empty``() =   [SetHeader (0, tid 8uy); SetEnd]                                -- 14 --> HashSet<int>()
    [<Test>] member __.``Map``() =          [MapHeader (1, tid 8uy, tid 11uy); Int32 1; String "x"; MapEnd] -- 13 --> dict([1, "x"])
    [<Test>] member __.``Map, empty``() =   [MapHeader (0, tid 8uy, tid 11uy); MapEnd]                      -- 13 --> Dictionary<int, string>()
    [<Test>] member __.``Array``() =        [ListHeader (1, tid 8uy); Int32 1; ListEnd]                     -- 15 --> [| 1 |]
    [<Test>] member __.``Array, empty``() = [ListHeader (0, tid 8uy); ListEnd]                              -- 15 --> Array.empty<int>
