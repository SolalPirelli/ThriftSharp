...
let MaxTimeout = TimeSpan.FromMilliseconds(100.0)
...
let ``Quick task``() = asTask <| async {
    ...
    let task = Task.Delay(5).ContinueWith(fun _ -> source.TrySetResult(42))
...
let ``Quickly faulted task``() = asTask <| async {
    let source = TaskCompletionSource<int>()
    let task = Task.Delay(5).ContinueWith(fun _ -> source.TrySetException(Exception("Oops")))
...
let ``Quickly canceled task``() = asTask <| async {
    let source = TaskCompletionSource<int>()
    let task = Task.Delay(5).ContinueWith(fun _ -> source.TrySetCanceled(CancellationToken(false)))
    do! Assert.ThrowsAnyAsync<OperationCanceledException>(fun () ->
...
let ``Very lengthy task``() = asTask <| async {
    let source = TaskCompletionSource<int>()
    Task.Delay(500).ContinueWith(fun _ -> source.TrySetResult(0)) |> ignore
    do! Assert.ThrowsAnyAsync<OperationCanceledException>(fun () ->
            TaskEx.TimeoutAfter(source.Task, MaxTimeout) :> Task) |> Async.AwaitTask |> Async.Ignore
}